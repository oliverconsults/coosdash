...
<div class="grid">
  ...
  <div>
    <?php
      // --- Costs widget (cooscrm.cost_events) ---
      // Small, always-visible overview for James/Oliver.
      try {
        $todayStart = date('Y-m-d 00:00:00');
        $weekStart  = date('Y-m-d 00:00:00', strtotime('-6 days')); // incl. today
        $monthStart = date('Y-m-01 00:00:00');

        $sumStmt = $pdo->prepare('SELECT COALESCE(SUM(COALESCE(cost_usd,0)),0) AS cost, COALESCE(SUM(COALESCE(tokens_in,0)),0) AS tin, COALESCE(SUM(COALESCE(tokens_out,0)),0) AS tout, COALESCE(SUM(COALESCE(requests,0)),0) AS req FROM cost_events WHERE ts >= ?');

        $sumStmt->execute([$todayStart]);
        $sumToday = $sumStmt->fetch() ?: ['cost'=>0,'tin'=>0,'tout'=>0,'req'=>0];

        $sumStmt->execute([$weekStart]);
        $sumWeek = $sumStmt->fetch() ?: ['cost'=>0,'tin'=>0,'tout'=>0,'req'=>0];

        $sumStmt->execute([$monthStart]);
        $sumMonth = $sumStmt->fetch() ?: ['cost'=>0,'tin'=>0,'tout'=>0,'req'=>0];

        $evStmt = $pdo->query('SELECT id, ts, node_id, provider, model, tokens_in, tokens_out, requests, cost_usd FROM cost_events ORDER BY ts DESC, id DESC LIMIT 10');
        $recentCosts = $evStmt ? $evStmt->fetchAll() : [];
      } catch (Throwable $e) {
        $sumToday = $sumWeek = $sumMonth = ['cost'=>0,'tin'=>0,'tout'=>0,'req'=>0];
        $recentCosts = [];
      }
    ...
    <div class="card" style="margin-bottom:12px">
      <div class="row" style="justify-content:space-between; align-items:center">
        ...
        <div class="meta">USD · Tokens · Requests</div>
      </div>

      <div class="row" style="gap:12px; flex-wrap:wrap; margin-top:8px">
        <div class="tag" style="background:#1a1f2a;">Heute: <?php echo h($fmtUsd($sumToday['cost'] ?? 0)); ?> · <?php echo (int)($sumToday['tin'] ?? 0) + (int)($sumToday['tout'] ?? 0); ?> tok · <?php echo (int)($sumToday['req'] ?? 0); ?> req</div>
        <div class="tag" style="background:#1a1f2a;">7 Tage: <?php echo h($fmtUsd($sumWeek['cost'] ?? 0)); ?> · <?php echo (int)($sumWeek['tin'] ?? 0) + (int)($sumWeek['tout'] ?? 0); ?> tok · <?php echo (int)($sumWeek['req'] ?? 0); ?> req</div>
        <div class="tag" style="background:#1a1f2a;">Monat: <?php echo h($fmtUsd($sumMonth['cost'] ?? 0)); ?> · <?php echo (int)($sumMonth['tin'] ?? 0) + (int)($sumMonth['tout'] ?? 0); ?> tok · <?php echo (int)($sumMonth['req'] ?? 0); ?> req</div>
      </div>

      <?php if (!empty($recentCosts)): ?>
        <div style="height:10px"></div>
        <div class="meta" style="margin-bottom:6px">Letzte Events</div>
        <div style="overflow:auto; max-height:260px">
          <table class="tbl" style="width:100%">
            ...
            <tbody>
              <?php foreach ($recentCosts as $r):
                $tok = (int)($r['tokens_in'] ?? 0) + (int)($r['tokens_out'] ?? 0);
                $nodeLink = ($r['node_id'] ? ('<a href="/?id=' . (int)$r['node_id'] . '">#' . (int)$r['node_id'] . '</a>') : '-');
                $m = trim((string)($r['provider'] ?? ''));
                $mm = trim((string)($r['model'] ?? ''));
                $modelLabel = trim($m . ' ' . $mm);
              ?>
                <tr>
                  <td><?php echo h(date('d.m H:i', strtotime((string)$r['ts']))); ?></td>
                  <td><?php echo $nodeLink; ?></td>
                  <td><?php echo h($modelLabel !== '' ? $modelLabel : '-'); ?></td>
                  <td style="text-align:right"><?php echo (int)$tok; ?></td>
                  <td style="text-align:right"><?php echo (int)($r['requests'] ?? 0); ?></td>
                  <td style="text-align:right"><?php echo h($fmtUsd($r['cost_usd'] ?? null)); ?></td>
                </tr>
              <?php endforeach; ?>
            </tbody>
          </table>
