...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if ($title === '' || $slug === '' || $desc === '') {
    ...
    // Build project setup tasks via LLM (use ONLY the prompt text stored in prompts.json)
    ...
    try {
      $cmd = 'clawdbot agent --session-id ' . escapeshellarg('coos-project-setup')
        . ' --message ' . escapeshellarg($setupPrompt)
      ...
      if (is_array($tmp)) {
        ...
        $err = 'LLM Setup: JSON ungültig.';
      }
    } catch (Throwable $e) {
      $err = 'LLM Setup fehlgeschlagen.';
    }
    ...
    if ($projectsId <= 0) {
      ...
      // Children: generated by LLM (project_setup_prompt)
      $children = [];
      $qualityTitle = 'Qualitätskontrolle';
      $qualityChildren = [];
      if (is_array($llmJson)) {
        if (!empty($llmJson['quality_title']) && is_string($llmJson['quality_title'])) {
          $qualityTitle = trim((string)$llmJson['quality_title']);
        }
        if (!empty($llmJson['children']) && is_array($llmJson['children'])) {
          foreach ($llmJson['children'] as $t) {
        ...
        if (!empty($llmJson['quality_children']) && is_array($llmJson['quality_children'])) {
          foreach ($llmJson['quality_children'] as $t) {
            if (!is_string($t)) continue;
            $t = trim($t);
            if ($t === '') continue;
            if (mb_strlen($t) > 40) $t = mb_substr($t, 0, 40);
      ...
      // Enforce rules: 4-6 children + last is qualityTitle
      ...
      // Remove any existing qualityTitle occurrences (we append it last)
      $children = array_values(array_filter($children, fn($t) => mb_strtolower($t) !== mb_strtolower($qualityTitle)));

      // Clamp size
      if (count($children) > 5) $children = array_slice($children, 0, 5);
      while (count($children) < 4) {
        $children[] = 'Implementierung (MVP)';
        if (count($children) >= 4) break;
      }
      $children[] = $qualityTitle;

      // Only create children if this is a fresh project node or the Ideen node has no children yet.
      $shouldCreateChildren = true;
      if ($fromNodeId > 0) {
        $stC = $pdo->prepare('SELECT COUNT(*) FROM nodes WHERE parent_id=?');
        $stC->execute([$parentId]);
        $shouldCreateChildren = ((int)$stC->fetchColumn() === 0);
      }
      ...
      if ($shouldCreateChildren) {
        ...
        foreach ($children as $ct) {
          ...
          if (mb_strtolower($ct) === mb_strtolower($qualityTitle)) {
            $qualityNodeId = $cid;
          }
        }

        // Add quality subpoints (5-10)
        if ($qualityNodeId > 0) {
          // Dedup + clamp
          $uq = [];
          $qc = [];
          foreach ($qualityChildren as $t) {
            $k = mb_strtolower($t);
            if (isset($uq[$k])) continue;
            $uq[$k] = true;
            $qc[] = $t;
          }
          if (count($qc) > 10) $qc = array_slice($qc, 0, 10);
          while (count($qc) < 5) $qc[] = 'Smoke-Test (kritische Pfade)';

          foreach ($qc as $qt) {
            $stIns->execute([$qualityNodeId, $qt, '', $createdBy, 'todo_james']);
          }
        }
      }
