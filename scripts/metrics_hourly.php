...
// Load minimal fields
$rows = $pdo->query("SELECT id, parent_id, title, worker_status FROM nodes ORDER BY id")->fetchAll();
...
$ideas = ['todo_oliver'=>0,'todo_james'=>0,'done'=>0];
$projects = ['todo_oliver'=>0,'todo_james'=>0,'done'=>0];
$laterTotal = 0;
$deletedTotal = 0;
...
(ts, ideas_todo_oliver, ideas_todo_james, ideas_done, projects_todo_oliver, projects_todo_james, projects_done, later_total, deleted_total)
VALUES
(?, ?, ?, ?, ?, ?, ?, ?, ?)
